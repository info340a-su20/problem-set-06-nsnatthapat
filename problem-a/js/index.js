...
function renderTrack(singleSong) {
  let divElem = document.querySelector("#records");
  let toAdd = document.createElement('img');
  toAdd.src = singleSong.artworkUrl100;
  toAdd.alt = singleSong.trackName;
  toAdd.title = singleSong.trackName;
  divElem.appendChild(toAdd);
}
...
function fetchTrackList(searchTerm) {
  ...
  let dataPromise = promise1.then(function(response) {
    let dataPromise = response.json();
    return dataPromise;
  });
  let rendered = dataPromise.then(renderSearchResults)
  
  let errorCheck = rendered.catch(function(err){
    renderError(err);
  });

  let toggled = errorCheck.then(togglerSpinner);
  return toggled;
}
